...
import club.tesseract.immortal.util.resetTeam
...
object ImmortalEvents {
    ...
    fun register(eventNode: EventNode<Event>) {
        ...
        eventNode.addListener(AsyncPlayerConfigurationEvent::class.java) { e ->
            val player = e.player
            ...
            if (isSpectating) {
                val playerToSpectate = MinecraftServer.getConnectionManager().getOnlinePlayerByUuid(UUID.fromString(args[2]))
                ...
                e.spawningInstance = game.instance!!
            } else {
...
//                Logger.info("game is joinable? ${newGame.canBeJoined(player)}")
                ...
                e.spawningInstance = newGame.instance!!
            }
        }
